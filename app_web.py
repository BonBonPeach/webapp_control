...
def leer_ingredientes_base(_ruta_archivo):
    ...
    if os.path.exists(_ruta_archivo):
        try:
            df = pd.read_csv(_ruta_archivo, encoding='latin-1')
            ...
            st.error(f"Error leyendo ingredientes: {e}")
    return ingredientes

def guardar_ingredientes_base(ruta_archivo, ingredientes_data):
    encabezados = ['Ingrediente', 'Proveedor', 'Unidad de Compra', ' Costo de Compra ', 
                   'Cantidad por Unidad de Compra', 'Unidad Receta', 'Costo por Unidad Receta']
    try:
        datos = []
        ...
        df.to_csv(ruta_archivo, index=False, encoding='latin-1')
        st.success("✅ Ingredientes guardados correctamente")
        st.cache_data.clear()  # Limpiar cache para refrescar datos
    except Exception as e:
        st.error(f"❌ Error guardando ingredientes: {e}")

def leer_inventario():
    """Leer inventario - SIN CACHE para evitar problemas de serialización"""
    inventario = {}
    if os.path.exists(ruta_inventario):
        try:
            df = pd.read_csv(ruta_inventario, encoding='latin-1')
            for _, fila in df.iterrows():
                nombre = str(fila.get('Ingrediente', '')).strip()
                if nombre:
                    inventario[nombre] = {
...
def guardar_inventario(ruta_archivo, inventario_data):
...
def leer_recetas():
    ...
    if os.path.exists(ruta_recetas):
        try:
            df = pd.read_csv(ruta_recetas, encoding='latin-1')
            ...
            st.error(f"Error leyendo recetas: {e}")
    
    return recetas

def leer_ventas(fecha_inicio=None, fecha_fin=None):
    """Leer ventas con filtro opcional por fechas"""
    ventas = []
    if os.path.exists(ruta_venta_diaria):
        try:
            df = pd.read_csv(ruta_venta_diaria, encoding='latin-1')
            if not df.empty:
            ...
            with col2:
                if ingrediente_existente:
                    if st.form_submit_button("🗑️ Eliminar Ingrediente", type="secondary"):
                        ...
                        guardar_ingredientes_base(ruta_ingredientes, ingredientes)
                        st.rerun()
...
def guardar_recetas_csv(recetas_data):
    """Guardar recetas en formato CSV"""
    try:
        ...
        df.to_csv(ruta_recetas, index=False, encoding='latin-1')
        st.cache_data.clear()  # Limpiar cache para refrescar
    except Exception as e:
...
def guardar_ventas_csv(ventas_data):
    """Guardar ventas en archivo CSV"""
    try:
        df = pd.DataFrame(ventas_data)
        df.to_csv(ruta_venta_diaria, index=False, encoding='latin-1')
...
def deducir_inventario_venta(producto, cantidad):
    ...
    try:
        ...
        guardar_inventario(ruta_inventario, inventario)
